    ...
    const deleteTodo = async (id) => {
      try {
        const response = await axios.delete(
          `http://localhost:4002/todo/delete/${id}`,
          {
            withCredentials: true,
          }
        );
    ...
    <div className=" my-10 bg-gray-100 max-w-lg lg:max-w-xl rounded-lg shadow-lg mx-8 sm:mx-auto p-6">
      ...
      <div className="flex mb-4">
        <input
          ...
          value={newTodo}
          onKeyUp={e=>e.key === "Enter" && createTodo(e)}
          onChange={(e)=>{setNewTodo(e.target.value)}}
          className="flex-grow p-2 border rounded-l-md focus:outline-none"
        />
        <button onClick={createTodo} className="bg-blue-600 border rounded-r-md text-white px-4 py-2 hover:bg-blue-900 duration-300">
          Add
        </button>
      </div>
      {loading ? (
